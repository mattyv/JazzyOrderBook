#pragma once
// select_nth.hpp
// Single-file implementation of "select": index of the n-th set bit in a 64-bit mask.
// - Runtime BMI2 detection on x86-64 (uses PDEP + TZCNT if available).
// - Portable fast fallback using byte lookup tables (works on x86-64 and ARM64).
//
// API:
//   int select_nth_set_bit(uint64_t mask, unsigned n);
//
// Behavior:
//   - Returns the bit index (0..63) of the n-th set bit in 'mask' (n is zero-based).
...
// - If BMI2 is present, we use _pdep_u64(1<<n, mask) and countr_zero to find the index.
// - If not, we use a 256-entry popcount-by-byte table and a 256x8 select-in-byte table.
...
// Build a 256-entry table where POP8[v] = number of 1-bits in the 8-bit value v.
// Computed at compile-time with constexpr; no runtime construction cost.
...
// Build a 256x8 table:
// For each 8-bit value v, SEL8[v][k] gives the bit index (0..7) of the k-th 1-bit in v,
// counting from least significant to most significant. If k is out of range for v,
// the entry is filled with 0xFF (unused).
...
inline int select_nth_set_bit_portable(uint64_t mask, unsigned n) {
    ...
    if (n >= total) throw std::out_of_range("n out of range");
...
inline bool cpu_has_bmi2() {
    // GCC/Clang on Linux: use __get_cpuid_count. If unavailable, conservatively return false.
    #if defined(__GNUC__) || defined(__clang__)
...
// - _pdep_u64 deposits the single 1-bit from (1 << n) into the n-th 1-bit position of 'mask'.
...
inline int select_nth_set_bit_bmi2(uint64_t mask, unsigned n) {
    ...
    // Route that bit through the 1-bit positions of mask: result has exactly one 1-bit.
